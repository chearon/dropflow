...
describe('Flow', function () {
  before(function () {
    ...
    this.reflow = function (html) {
      ...
      flow.reflow(this.layout, 300, 500);
  ...
  describe('Box generation', function () {
    it('wraps inlines in block boxes', function () {
      this.reflow('<div><span>abc</span><div></div>def</div>');
    ...
    it('breaks out block level elements', function () {
      ...
      expect(this.get(0, 3).treeStart).to.equal(14);
      expect(this.get(0, 3).treeFinal).to.equal(24);
      // <anon div><span> 2out</span></anon div>
      expect(this.get(0, 3, 0).isBlockContainer()).be.true
      expect(this.get(0, 3, 0).isAnonymous()).be.true
      // end
      expect(this.get(0).treeStart).to.equal(1);
      expect(this.get(0).treeFinal).to.equal(27);
    });
    ...
    it('generates BFCs', function () {
      this.reflow('<div style="display: flow-root;"></div>');
    ...
    it('doesn\'t create block boxes with display: none', function () {
      ...
      expect(this.get(0).treeStart).to.equal(1);
      expect(this.get(0).treeFinal).to.equal(12);
    });
    ...
    it('doesn\'t create inline boxes with display: none', function () {
      ...
      const block = this.get(0);
      expect(this.get(0).getContentArea().height).to.equal(100);
      expect(block.text).to.equal(' shown ');
    });

    it('generates nothing for <br> with display: none', function () {
      this.reflow('abc <br style="display: none"> def');
      expect(this.layout.tree.length).to.equal(4);
      expect(this.layout.tree[2].isRun()).to.be.true;
      expect(this.layout.tree[3].isRun()).to.be.true;
    });

    it('generates nothing for display: none; and float', function () {
      this.reflow('<div style="float: right; display: none;"></div>');
      expect(this.get().treeStart).to.equal(this.get().treeFinal);
    });
  });
  ...
  describe('Collapsing', function () {
    it('collapses through, sets heights, offsets correctly', function () {
      ...
      expect(this.layout.root().getContentArea().y).to.equal(0);
  ...
  describe('Units', function () {
    ...
    it('resolves em units on width and height', function () {
      this.reflow(`<div style="width: 1em; height: 1em;"></div>`);
  ...
  describe('Floats', function () {
    it('can be the exclusive content', function () {
      ...
      /** @type import('../src/layout-flow.ts').BlockContainerOfBlocks */
      const box = this.get(0, 0, 0);
    ...
    it('sets non-bfc containing block height to zero if it has only floats', function () {
      ...
      /** @type import('../src/layout-flow.ts').BlockContainer */
      const bfc = this.get('#t1');
      expect(bfc.getContentArea().height).to.equal(70);
      /** @type import('../src/layout-flow.ts').BlockContainer */
      const cb = this.get('#t2');
    ...
    it('places zero height floats after normal floats', function () {
      ...
      /** @type import('../src/layout-flow.ts').BlockContainerOfInlines */
      expect(this.layout.tree[4].getBorderArea().x).to.equal(0);
      expect(this.layout.tree[5].getBorderArea().x).to.equal(0);
      expect(this.layout.tree[6].getBorderArea().x).to.equal(10);
      expect(this.layout.tree[7].getBorderArea().x).to.equal(10);
      expect(this.layout.tree[8].getBorderArea().x).to.equal(20);
      expect(this.layout.tree[9].getBorderArea().x).to.equal(20);
      expect(this.layout.tree[10].getBorderArea().x).to.equal(30);
    });
    ...
    it('obeys rule 1', function () {
      this.reflow(`
        <div id="t1" style="width: 100px;">
          <div id="t2" style="width: 25px; height: 25px; float: left;"></div>
          <div id="t3" style="width: 25px; height: 25px; float: right;"></div>
          hey!
        </div>
      `);
    ...
    it('obeys rule 2', function () {
      this.reflow(`
        <div style="width: 100px;">
          <div id="t1" style="width: 25px; height: 25px; float: left;"></div>
          <div id="t2" style="width: 25px; height: 25px; float: left;"></div>
        </div>
      `);

      /** @type import('../src/layout-flow.ts').BlockContainerOfBlocks */
      const left1 = this.get('#t1');
      /** @type import('../src/layout-flow.ts').BlockContainerOfBlocks */
      const left2 = this.get('#t2');
    ...
    it('obeys rule 3', function () {
      this.reflow(`
        <div style="width: 100px;">
          xx
          <div id="t1" style="width: 51px; height: 50px; float: left;"></div>
          <div id="t2" style="width: 50px; height: 50px; float: right;"></div>
        </div>
      `);
    ...
    it('obeys rule 4', function () {
      this.reflow(`
        <div style="width: 100px; line-height: 20px;">
          has space
          <div><div id="t" style="width: 10px; height: 10px; float: left;"></div></div>
        </div>
      `);
    ...
    it('obeys rule 6', function () {
      this.reflow(`
        <div style="width: 200px; font: 16px/20px Arimo;">
          <div id="t1" style="width: 20px; height: 20px; float: left;"></div>
          As the text flows down, potential float positions flow down too.
          <div id="t2" style="width: 20px; height: 20px; float: right;"></div>
        </div>
      `);

      /** @type import('../src/layout-flow.ts').BlockContainerOfBlocks */
      const float1 = this.get('#t1');
      /** @type import('../src/layout-flow.ts').BlockContainerOfBlocks */
      const float2 = this.get('#t2');
      expect(float1.getBorderArea().x).to.equal(0);
      expect(float1.getBorderArea().y).to.equal(0);
    ...
    it('obeys rule 7', function () {
      this.reflow(`
        <div style="width: 200px; font: 16px/20px Arimo;">
          ...
          <div id="t" style="float: left; width: 300px; height: 5px;"></div>
    ...
    it('obeys rule 8', function () {
      this.reflow(`
        <div style="width: 200px; font: 16px/20px Arimo;">
          Hello there <div id="t" style="float: left; width: 5px; height: 5px;"></div>
        </div>
      `);
    ...
    it('obeys rule 9', function () {
      this.reflow(`
        <div style="width: 200px; font: 16px/20px Arimo;">
          Hello there
          <div id="t1" style="float: left; width: 5px; height: 5px;"></div>
          <div id="t2" style="float: left; width: 5px; height: 5px;"></div>
        </div>
      `);
    ...
    it('obeys rule 10', function () {
      this.reflow(`
        <div style="width: 200px; font: 16px/20px Arimo;">
          ...
          Lorem
          <div id="t1" style="width: 20px; height: 20px; float: left; clear: left;"></div>
          ipsum text is one way to generate filler text,
          <div id="t2" style="width: 20px; height: 20px; float: right;"></div>
          but
          <div id="t3" style="width: 20px; height: 20px; float: left; clear: right;"></div>
          another way is stream of consciousness.
        </div>
      `);

      /** @type import('../src/layout-flow.ts').BlockContainerOfBlocks */
      const float2 = this.get('#t1');
      /** @type import('../src/layout-flow.ts').BlockContainerOfBlocks */
      const float3 = this.get('#t2');
      /** @type import('../src/layout-flow.ts').BlockContainerOfBlocks */
      const float4 = this.get('#t3');
      expect(float2.getBorderArea().x).to.equal(0);
      expect(float2.getBorderArea().y).to.equal(40);
  ...
  describe('Zoom', function () {
    ...
    it('zooms natural image size', function () {
      this.reflow(`<img src="${adaUrl}" style="zoom: 200%;">`);
  ...
  describe('Images', function () {
    ...
    it('sizes width based on height', function () {
      this.reflow(`<img src="${adaUrl}" style="height: 100px;">`);
    ...
    it('correctly positions text after sized images without content', function () {
      ...
      /** @type import('../src/layout-flow.ts').BlockContainerOfInlines */
      const block = this.get('div');
      expect(block.items[4].x).to.equal(122.236328125);
      expect(block.items[4].y).to.equal(25);
    });

    it('correctly positions text after unsized images without content', function () {
      this.reflow('<div style="font-size: 10px;">no<img>oo');
      /** @type import('../src/layout-flow.ts').BlockContainerOfInlines */
      const block = this.get('div');
      expect(block.items[1].x).to.equal(11.123046875);
    });
    ...
    it('displays images inline', function () {
      this.reflow(`<img src="${adaUrl}" style="width: 100px; height: 100px; margin: 10px;">dog`);
      /** @type import('../src/layout-flow.ts').BlockContainerOfInlines */
      const block = this.get();
      expect(block.items[0].x).to.equal(120);
      expect(block.items[0].y).to.equal(120);
    });
    ...
    it('displays images as block', function () {
      this.reflow(`<div style="width: 1000px;"><img src="${adaUrl}" style="display: block;">`);
      expect(this.get('img').getContentArea().x).to.equal(0);
      expect(this.get('img').getContentArea().y).to.equal(0);
      expect(this.get('img').getContentArea().width).to.equal(369);
      expect(this.get('img').getContentArea().height).to.equal(376);
      this.reflow(`<div style="width: 1000px; direction: rtl;"><img src="${adaUrl}" style="display: block;">`);
