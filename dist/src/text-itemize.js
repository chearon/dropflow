...
const { 
// SheenBidi
SBAlgorithmCreate, SBAlgorithmRelease, SBAlgorithmGetParagraphBoundary, SBAlgorithmCreateParagraph, SBParagraphRelease, SBParagraphGetLevelsPtr, 
// emoji-segmenter
emoji_scan, malloc, free, memory } = wasm.instance.exports;
...
export function createBidiIteratorState(stringPtr, stringLength, initialLevel = 0) {
...
// exported for testing
export function bidiIteratorNext(state) {
    if (state.done)
        return;
    ...
    outer: while (state.offset < state.stringLength) {
        if (state.offset === state.paragraphEnd) {
            if (state.paragraphPtr)
                SBParagraphRelease(state.paragraphPtr);
            heapu32[paraLenPtr32] = 0;
            heapu32[paraSepPtr32] = 0;
            SBAlgorithmGetParagraphBoundary(state.algorithmPtr, state.offset, state.stringLength - state.offset, paraLenPtr, paraSepPtr);
            ...
            state.paragraphPtr = SBAlgorithmCreateParagraph(state.algorithmPtr, state.offset, paraLen, state.initialLevel);
            state.levelsPtr = SBParagraphGetLevelsPtr(state.paragraphPtr);
            if (state.offset === 0)
                state.level = heapu8[state.levelsPtr];
        }
        while (state.offset < state.paragraphEnd) {
            if (heapu8[state.levelsPtr + state.offset - state.paragraphStart] !== state.level)
                break outer;
            state.offset += 1;
        }
    }
...
export function createEmojiIteratorState(stringPtr, stringLength) {
...
export function emojiIteratorNext(state) {
    if (state.done)
        return;
    ...
    while (p < end) {
        ...
        if (state.index === 0)
            state.isEmoji = isEmoji;
        state.index = p - state.typesPtr;
        if (isEmoji !== state.isEmoji)
            return;
        state.offset = state.offsets[state.index];
    }
...
export function createScriptIteratorState(stringPtr, stringLength) {
    return {
        offset: 0,
...
export function scriptIteratorNext(state) {
    if (state.done)
        return;
    state.script = 'Common';
    ...
    while (state.offset < state.stringLength) {
        ...
        if (isSame) {
            if (!runningIsReal && isReal) {
                ...
                while (parens[state.startParen + 1])
                    parens[++state.startParen].script = script;
...
export function createNewlineIteratorState(str) {
    return { offset: 0, str, done: false };
}
export function newlineIteratorNext(state) {
    if (state.done)
        return;
...
export function createStyleIteratorState(ifc) {
    return {
        parents: [ifc],
...
export function styleIteratorNext(state) {
    if (state.done)
        return;
    state.lastOffset = state.offset;
    if (state.leader !== END_CHILDREN) {
        state.style = state.leader.style;
        if (state.leader.isRun())
            state.offset += state.leader.length;
    }
    while (state.stack.length) {
        ...
        if (item === END_CHILDREN) {
            ...
            if (state.style.fontSize !== item.style.fontSize ||
    ...
    if (state.offset === state.ifc.text.length)
        state.done = true;
}
export function createItemizeState(ifc) {
    let newlineState;
    let inlineState;
    let emojiState;
    let bidiState;
    ...
    if (ifc.isComplexText()) {
        ...
        for (let i = 0; i < ifc.text.length; i++)
            array[i] = ifc.text.charCodeAt(i);
...
export function itemizeNext(state) {
    if (state.done)
        return;
    if (state.simple) {
    ...
    // Advance
    if (newlineState?.offset === offset)
        newlineIteratorNext(newlineState);
    if (inlineState?.offset === offset)
        styleIteratorNext(inlineState);
    if (emojiState?.offset === offset)
        emojiIteratorNext(emojiState);
    if (bidiState?.offset === offset)
        bidiIteratorNext(bidiState);
    if (scriptState?.offset === offset)
        scriptIteratorNext(scriptState);
    // Map the current iterators to context
    if (inlineState)
        state.attrs.style = inlineState.style;
    if (emojiState)
        state.attrs.isEmoji = emojiState.isEmoji;
    if (bidiState)
        state.attrs.level = bidiState.level;
    if (scriptState)
        state.attrs.script = scriptState.script;
    state.offset = Math.min(newlineState?.offset ?? Infinity, inlineState?.offset ?? Infinity, emojiState?.offset ?? Infinity, bidiState?.offset ?? Infinity, scriptState?.offset ?? Infinity, state.length);
    if ((!newlineState || newlineState.done) &&
