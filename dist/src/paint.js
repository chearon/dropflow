import { ShapedItem, G_CL, G_AX, G_SZ } from './layout-text.js';
...
function drawText(containingBlock, item, colors, b) {
 ...
 */
function paintBlockBackground(block, b, isRoot = false) {
...
function paintBackgroundDescendents(root, b) {
    ...
    while (stack.length) {
        ...
        for (let i = block.children.length - 1; i >= 0; i--) {
            ...
            if (child.isBox() && !child.isPaintRoot())
                stack.push(child);
        }
    }
}
...
function paintInlinesAndDescendents(root, b) {
    const stack = [root];
    while (stack.length) {
        const box = stack.pop();
...
function collectLayeredDescendents(box, paragraph) {
    ...
    negativeRoots.sort(([a], [b]) => a.style.zIndex - b.style.zIndex);
    positiveRoots.sort(([a], [b]) => a.style.zIndex - b.style.zIndex);
    return { negativeRoots, floats, positionedBoxes, positiveRoots };
}
function paintLayeredDescendents(descendents, b) {
...
function paintInline(inline, paragraph, b) {
...
function paintInlineRoot(inline, paragraph, b) {
    const { negativeRoots, floats, positionedBoxes, positiveRoots } = collectLayeredDescendents(inline, paragraph);
 ...
 */
export default function paintBlockRoot(block, b, isRoot = false) {
    const { negativeRoots, floats, positionedBoxes, positiveRoots } = collectLayeredDescendents(block, undefined);
