...
import { Style, initialStyle, createComputedStyle } from './cascade.js';
import { createInlineIterator, createPreorderInlineIterator, IfcVacancy, layoutFloatBox } from './flow.js';
...
function isSgUncollapsible(whiteSpace) {
...
export class Collapser {
    ...
    mod(start, end, s) {
        ...
        for (let k = rstart; k < this.runs.length; ++k) {
            ...
            if (k <= rend)
                shrinkahead += run.mod(start, end - shrinkahead, s);
    ...
    *collapsibleRanges(filter) {
        ...
        while (true) {
            ...
            if (wasInCollapse && !isInCollapse)
                yield [this.runs[start], this.runs[end - 1]];
            if (over)
                break;
...
export const G_FL = 6;
export const G_SZ = 7;
function extractHbBufferGlyphs(buf) {
...
class ShapedShim {
    constructor(offset, inlines, attrs) {
        this.offset = offset;
...
export function baselineStep(parent, inline) {
...
export class Linebox extends LineItemLinkedList {
    ...
    trimEnd() {
        for (let n = this.tail; n; n = n.previous) {
            if (n.value instanceof ShapedShim)
                continue;
            if (n.value.collapseWhitespace('end'))
                return;
        }
    }
...
function createIfcBuffer(text) {
    ...
    for (let i = 0; i < text.length; ++i) {
        ...
        if (c == zeroWidthNonJoinerCharacter
...
function createWordCacheKey(font, attrs) {
    let hash = hashMix(font.ptr, attrs.level & 1);
    for (let i = 0; i < attrs.script.length; ++i) {
        hash = hashMix(hash, attrs.script.charCodeAt(i));
    }
    return hash;
}
    ...
    *itemize() {
        ...
        if (this.ifc.isComplexText()) {
            ...
            for (let i = 0; i < this.string.length; i++)
                buf[i] = this.string.charCodeAt(i);
        ...
        while ((!emoji || !emoji.done) &&
            (!bidi || !bidi.done) &&
            (!script || !script.done) &&
            (!style || !style.done) &&
            !newline.done) {
            // Find smallest text index
            const smallest = Math.min(emoji?.value.i ?? Infinity, bidi?.value.i ?? Infinity, script?.value.i ?? Infinity, style?.value.i ?? Infinity, newline.value.i);
            ...
            // Advance
            if (emoji && smallest === emoji.value.i)
                emoji = iEmoji.next();
            if (bidi && smallest === bidi.value.i)
                bidi = iBidi.next();
            if (script && smallest === script.value.i)
                script = iScript.next();
            if (style && smallest === style.value.i)
                style = iStyle.next();
            if (smallest === newline.value.i)
                newline = iNewline.next();
            yield { i: smallest, attrs: ctx };
        }
        free();
    }
    shapePart(offset, length, font, attrs) {
        const end = offset + length;
        const words = [];
        let size = 0;
        let hash = createWordCacheKey(font, attrs);
        let wordLen = 0;
        let wordStart = offset;
        buf.guessSegmentProperties();
        for (let i = offset; i <= end; ++i) {
            if (this.string[i] === ' ' || i === end && wordLen > 0) {
                let wordGlyphs = wordCache.get(hash);
                if (!wordGlyphs) {
                    if (wordCache.size > 10_000)
                        wordCache.clear();
                    buf.setLength(0);
                    buf.addUtf16(this.buffer.array.byteOffset + wordStart * 2, wordLen, 0, wordLen);
                    hb.shape(font, buf);
                    wordGlyphs = extractHbBufferGlyphs(buf);
                    wordCache.set(hash, wordGlyphs);
                }
                words.push({ wordStart, wordGlyphs });
                size += wordGlyphs.length;
                hash = createWordCacheKey(font, attrs);
                wordStart = i;
                wordLen = 0;
            }
            hash = hashMix(hash, this.string.charCodeAt(i));
            wordLen += 1;
        }
    ...
    createLineboxes(ctx) {
        ...
        let itemInMark; // TODO: merge with item?
        ...
        for (const mark of { [Symbol.iterator]: () => this.createMarkIterator() }) {
            ...
            if (mark.isItemEnd)
                itemInMark = undefined;
            if (mark.inlinePre) {
                candidates.height.pushInline(mark.inlinePre);
                if (itemInMark)
                    candidates.height.stampMetrics(getMetrics(mark.inlinePre.style, itemInMark.match));
            ...
            if (inkAdvance || !wsCollapsible)
                unbreakableMark = mark.position;
            ...
            if (mark.isBreak && (lineHasInk && !nowrap || mark.isBreakForced || mark.position === this.length())) {
                ...
                bfc.getLocalVacancyForLine(blockOffset, blockSize, vacancy);
