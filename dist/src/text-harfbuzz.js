    ...
    [Symbol.iterator]() {
        ...
        const next = () => {
            if (exports.hb_set_next(this.ptr, valuePtr)) {
                return { value: heapu32[valuePtr >>> 2], done: false };
            }
            else {
                return { done: true };
            }
...
export function wrapExternalSet(ptr) {
...
export class HbFace {
    ...
    getScripts() {
        ...
        do {
            exports.hb_ot_layout_table_get_script_tags(this.ptr, HB_OT_TAG_GSUB, offset, lengthPtr, tagsPtr);
    ...
    getFeatureIndexes(table, scriptIndex, langIndex) {
        ...
        heapu32[lengthPtr >> 2] = maxLength;
        do {
            exports.hb_ot_layout_language_get_feature_indexes(this.ptr, table, scriptIndex, langIndex, offset, lengthPtr, featureIndexesPtr);
            length = heapu32[lengthPtr >> 2];
            for (let i = 0; i < length; i++) {
    ...
    getRequiredFeatureIndex(table, scriptIndex, langIndex) {
        const featurePtr = bytes4;
        if (exports.hb_ot_layout_language_get_required_feature_index(this.ptr, table, scriptIndex, langIndex, featurePtr)) {
    ...
    getFeatureTags(table, scriptIndex, langIndex) {
        const lengthPtr = bytes4;
        const maxLength = 32;
        ...
        heapu32[lengthPtr >> 2] = maxLength;
        do {
            exports.hb_ot_layout_language_get_feature_tags(this.ptr, table, scriptIndex, langIndex, offset, lengthPtr, featureTagsPtr);
            length = heapu32[lengthPtr >> 2];
            for (let i = 0; i < length; i++) {
    ...
    getLookupsByFeature(table, featureIndex, lookups) {
        const lengthPtr = bytes4;
        const maxLength = 32;
        ...
        heapu32[lengthPtr >> 2] = maxLength;
        do {
            exports.hb_ot_layout_feature_get_lookups(this.ptr, table, featureIndex, offset, lengthPtr, lookupsPtr);
            length = heapu32[lengthPtr >> 2];
            for (let i = 0; i < length; i++) {
    ...
    collectGlyphs(table, lookupIndex, beforeGlyphs, inputGlyphs, afterGlyphs, outputGlyphs) {
        exports.hb_ot_layout_lookup_collect_glyphs(this.ptr, table, lookupIndex, beforeGlyphs?.ptr ?? 0, inputGlyphs?.ptr ?? 0, afterGlyphs?.ptr ?? 0, outputGlyphs?.ptr ?? 0);
    }
    destroy() {
...
export class HbBuffer {
    ...
    setLanguage(language) {
        ...
        for (let i = 0; i < len; i++)
            heapu8[langPtr + i] = language.codePointAt(i);
