...
const emojiTrie = createTrie(wasm.instance.exports.memory.buffer, wasm.instance.exports.emoji_trie.value);
const scriptTrie = createTrie(wasm.instance.exports.memory.buffer, wasm.instance.exports.script_trie.value);
const { 
// SheenBidi
...
export function* bidiIterator(str, initialLevel = 0) {
    ...
    while (offset < str.length) {
        ...
        if (isFirstParagraph)
            lastLevel = levels[0];
        if (isLastParagraph)
            j += 1; /* check levels[levels.length] to emit the final character */
        for (let i = 0; i < j; ++i) {
            const level = levels[i];
            if (level !== lastLevel)
                yield { i: offset + i, level: lastLevel };
            lastLevel = level;
        }
...
export function* emojiIterator(str) {
    const types = [];
    const offsets = [];
...
export function* scriptIterator(text) {
    ...
    while (scriptEnd < textEnd) {
        ...
        if (isSame) {
            if (!runningIsReal && isReal) {
                ...
                while (parens[startParen + 1])
                    parens[++startParen].script = script;
